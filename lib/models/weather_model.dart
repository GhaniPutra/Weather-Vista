...
class WeatherModel {
  ...
  /// Daftar perkiraan harian (forecast) — jika tersedia dari API.
  final List<DailyForecast> forecastDays;

  /// Konstruktor dengan semua parameter yang diperlukan
  /// `forecastDays` bersifat opsional (default kosong) agar kompatibel dengan
  /// pembuatan model manual di tes atau panggilan sebelumnya.
  WeatherModel({
    required this.tempC,
  ...
  factory WeatherModel.fromJson(Map<String, dynamic> json) {
    ...
    /// Parse seluruh array forecastday menjadi DailyForecast list (jika tersedia)
    final List<dynamic> forecastList = json['forecast']?['forecastday'] ?? [];
    final List<DailyForecast> forecastDays = forecastList.map((d) {
      final dayObj = d['day'] ?? {};
      return DailyForecast(
        date: d['date'] ?? '',
        minTemp: (dayObj['mintemp_c'] ?? 0).toDouble(),
        maxTemp: (dayObj['maxtemp_c'] ?? 0).toDouble(),
        conditionText: dayObj['condition']?['text'] ?? '',
        iconUrl: 'https:${dayObj['condition']?['icon'] ?? ''}',
      );
    }).toList();
    ...
    return WeatherModel(
      ...
      /// Daftar perkiraan harian (bisa kosong jika API tidak mengirim)
...
/// Model untuk data prakiraan harian (sintesis dari API forecastday)
class DailyForecast {
  /// Tanggal dalam format YYYY-MM-DD
  final String date;

  /// Suhu minimum hari ini (Celsius)
  final double minTemp;

  /// Suhu maksimum hari ini (Celsius)
  final double maxTemp;

  /// Deskripsi kondisi harian (contoh: 'Light rain')
  final String conditionText;

  /// Icon URL untuk kondisi (full https URL)
  final String iconUrl;
